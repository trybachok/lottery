...
Найдите созданный тираж со статусом `DRAFT`, `SCHEDULED`, `PAUSED` или `POSTPONED` и нажмите `Activate`.
...
Активировать тираж можно сразу в интерфейсе:
...
3. Создать тираж.
4. В строке созданного тиража нажать `Activate`.
```

Если кнопка `Activate` заблокирована, проверьте, что текущий пользователь является `ADMIN` или назначенным `MANAGER` этого тиража.

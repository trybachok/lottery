...
Миграции создают роли и permissions, но не создают пользователя-администратора. Для локальной разработки первый admin создается так:
...
4. Назначьте этому пользователю роль `ADMIN` через PostgreSQL:

```bash
docker compose --env-file .env.example exec postgres psql -U lottery -d lottery -c "insert into user_roles (user_id, role_id) select u.id, r.id from users u join roles r on r.code = 'ADMIN' where u.email = 'admin@example.com' on conflict do nothing;"
```

5. Выйдите из системы и войдите снова под этим пользователем.

После повторного входа будут доступны:
...
- Первый admin пользователь не seed-ится миграциями, его нужно назначить вручную для локальной разработки.

...
Миграции создают роли и permissions, но не создают пользователя-администратора. Первый администратор создается автоматически, если самый первый зарегистрированный пользователь имеет логин `owner`.
...
Важно: автоматическое назначение `ADMIN` работает только для самого первого пользователя в пустой базе и только при логине `owner`. Все остальные регистрации получают роль `CLIENT`.
